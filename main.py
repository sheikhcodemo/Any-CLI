...
@app.command()
def fine_tune(
    dataset_path: str = typer.Option("data.json", help="The path to the dataset."),
    model_name: str = typer.Option("distilgpt2", help="The name of the model to fine-tune."),
    output_dir: str = typer.Option("./fine-tuned-model", help="The directory to save the fine-tuned model to."),
):
    """
    Fine-tune a pre-trained model on a given dataset.
    """
    try:
        fine_tune_model(dataset_path, model_name, output_dir)
    except Exception as e:
        typer.echo(f"ERROR: An unexpected error occurred during fine-tuning: {e}", err=True)
        raise typer.Exit(code=1)
